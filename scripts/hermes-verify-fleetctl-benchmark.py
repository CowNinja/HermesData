#!/usr/bin/env python3
"""Ad-hoc verification: fleetctl promote + swap + benchmark_harness.
Runs dry-mode checks for all new behavioral paths. No side effects on
running server or model files."""
...
def load_module(name):
    p = module_path(name)
    if not p:
        skip(f"{name}.py not found")
        return None
    spec = importlib.util.spec_from_file_location(name, p)
    m = importlib.util.module_from_spec(spec)
    try:
        spec.loader.exec_module(m)
        ok(f"{name}.py parses and imports cleanly")
        return m
    except Exception as e:
        fail(f"{name}.py import error: {e}")
        return None
...
if out:
    expected_cmds = ["promote", "swap", "suggest", "benchmark",
                     "reconcile", "scan", "list", "status", "info"]
    missing = [c for c in expected_cmds if c not in out]
    if missing:
        fail(f"Missing commands in --help: {missing}")
    else:
...
if out2:
    lines = [l for l in out2.splitlines() if l.strip() and "unbenchmarked" in l.lower()]
    if lines:
        ok(f"Candidate enumeration: {lines[0].strip()}")
    # Check it returns numeric count
    import re
    nums = re.findall(r'\d+', out2)
...
if out3:
    # Should name Qwythos or another candidate, not error about missing benchmark
    if "error" in out3.lower() and "benchmark" in out3.lower():
        fail(f"suggest raises benchmark error: {out3[:200]}")
    else:
...
if out5:
    if "candidate" in out5.lower() or "benchmark" in out5.lower() or "lifecycle" in out5.lower():
        ok("info returns lifecycle/benchmark data")
    else:
...
if bench_dir.exists():
    ...
    jsons = list(bench_dir.glob("*.json"))
    if jsons:
        ok(f"Found {len(jsons)} benchmark JSON files")
        for jf in jsons:
            try:
...
# Exit code
sys.exit(1 if VERDICT["fail"] > 0 else 0)
