#!/usr/bin/env python3
"""
VaultWalker v0.6.2 - Rebuilt clean, explicit-only gates, rock solid.
...
def local_model_file_evaluation(preview: str, path_str: str, current_silo: str) -> Dict:
    """Use existing qwythos 9b abliterated (loaded in sovereign router; no reinvention).
    Intelligent classification + recommended action.
    Block by default. Only explicit-rp triggers central move to sandbox.
    """
    model = "qwythos-9b-abliterated"
    prompt = (
    ...
    try:
        result = subprocess.run(["ollama", "run", model, prompt], capture_output=True, text=True, timeout=70)
        ...
        return {"class": "unknown", "action": "keep", "reason": "model unavailable", "residency": "keep-current", "policy": "no-cross-except-explicit"}
...
def update_per_folder_indexes(root: Path, silo_name: str, state: Dict[str, Any], is_light: bool) -> int:
    ...
    for dirpath in root.rglob("*"):
        ...
        content = f"""# {dirpath.name} INDEX (VaultWalker v0.6.2, {datetime.now().strftime('%Y-%m-%d')}) Silo: {silo_name}
...
def evaluate_and_relocate_misplaced(root: Path, silo_name: str, dry_run: bool = False) -> Dict:
    ...
    for p in root.rglob("*"):
        if not p.is_file():
            continue
        pstr = str(p).lower()
        if any(x in pstr for x in [".git", "node_modules", "__pycache__", "tmp", ".bak", "backup"]):
            continue
...
def main():
    global args, DRY_RUN
    parser = argparse.ArgumentParser(description="VaultWalker v0.6.2 - explicit-only silo housekeeper")
