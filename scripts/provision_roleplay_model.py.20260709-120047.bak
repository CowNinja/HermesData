#!/usr/bin/env python3
"""Download and register the uncensored roleplay GGUF for local_roleplay tier."""
...
def _download(repo: str, filename: str) -> dict:
    cmd = [
        sys.executable,
...
def _register_model(path: Path, logical: str, cfg: dict) -> None:
    model_cfg = cfg.get("model") or {}
    entry = {
        "gguf_file": path.name,
        "path": str(path),
        "role": "uncensored_roleplay",
        "latency_budget_ms": 90000,
        "tier": "local_roleplay",
        "port": 8090,
        "storage_tier": "candidate" if "candidates" in str(path) else "current",
        "uncensored": True,
        "ctx_size": model_cfg.get("ctx_size", 12288),
    }
    if GGUF_MAP.is_file():
        ...
        data["reconciled_at"] = __import__("datetime").datetime.now(__import__("datetime").timezone.utc).isoformat()
    ...
    if MODELS_INI.is_file():
        ...
        if f"[{logical}]" not in text:
            MODELS_INI.write_text(text.rstrip() + ini_block, encoding="utf-8")

    if MOE_MAP.is_file():
        moe = json.loads(MOE_MAP.read_text(encoding="utf-8-sig"))
        ur = moe.setdefault("unified_router", {})
        ur.setdefault("tier_logical_models", {})["local_roleplay"] = logical
...
def main() -> int:
    ...
    if not result.get("ok"):
        print(json.dumps({"status": "download_failed", **result}, indent=2))
        return 1
    ...
    logical = cfg.get("logical_model", "rocinante-12b")
    if "abliterated" in path.name.lower():
        logical = "llama-3.1-8b-abliterated"

    _register_model(path, logical, cfg)
    print(json.dumps({"status": "provisioned", "path": str(path), "logical_model": logical}, indent=2))
    return 0
