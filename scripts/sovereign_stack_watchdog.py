...
# Rotate before append so 60s ticks cannot grow a multi-GB jsonl (audit 2026-07-18: ~9.3MB unbound).
_WATCHDOG_LOG_MAX_BYTES = 8 * 1024 * 1024  # 8 MiB
...
def _rotate_log_if_needed(path: Path, max_bytes: int = _WATCHDOG_LOG_MAX_BYTES, backups: int = _WATCHDOG_LOG_BACKUPS) -> None:
    """Rotate path -> path.1 .. path.N when over max bytes (shared rotator preferred)."""
    try:
        from jsonl_log_rotator import rotate_if_needed

        rotate_if_needed(path, max_bytes=max_bytes, backups=backups, mode="rename")
        return
    except Exception:
        pass
    try:
    ...
    try:
        try:
            ...
            _rot_append(
                WATCHDOG_LOG,
                event,
                max_bytes=_WATCHDOG_LOG_MAX_BYTES,
                backups=_WATCHDOG_LOG_BACKUPS,
                mode="rename",
                stamp=True,
            )
            return
        ...
        _rotate_log_if_needed(WATCHDOG_LOG)
