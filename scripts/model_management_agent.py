...
try:
    ...
    def _annotate_issues(issues):  # type: ignore
        return issues
...
def _log(event: Dict[str, Any]) -> None:
    try:
        ...
        _rot_append(LOG_PATH, event, mode="rename", stamp=True)
        return
    except Exception:
        pass
...
def _operator_commands(issue_codes: List[str]) -> Dict[str, str]:
    ...
    # SSOT: Python bridge entries preferred (Hermes no_agent). PS wrappers remain for manual ops.
    cmds = {
        ...
        "cron_light": f'"{py}" D:\\HermesData\\scripts\\model_mgmt_light_cron.py',
        "cron_full": f'"{py}" D:\\HermesData\\scripts\\model_mgmt_full_cron.py',
        "cron_light_ps": r"D:\HermesData\scripts\model_management_cron.ps1 -Mode light",
        "cron_full_ps": r"D:\HermesData\scripts\model_management_cron.ps1 -Mode full",
        "summary": f'"{py}" D:\\HermesData\\scripts\\model_management_agent.py --tick --summary',
        "benchmark_active": f'"{py}" D:\\HermesData\\scripts\\model_management_agent.py --benchmark-active --summary',
        "rotate_logs": f'"{py}" D:\\HermesData\\scripts\\jsonl_log_rotator_cron.py',
        ...
        "hermes_jobs": "Model-Mgmt-Light-Tick 5afddd546ec2 | Full 5f73c5cf061a | JSONL-Log-Rotator-6h c803d7ac3261",
    }
...
def _parse_session_health_patterns() -> List[str]:
    """Extract recent failure patterns from Session-Health-Log for agent reflection.

    Only the last ~16KB is scanned. The vault Session-Health-Log is cumulative;
    whole-file substring matches permanently false-positive (proxy/fork/drift)
    and polluted next_tick_hints even when the live stack was GREEN (audit 2026-07-18).
    """
...
def _self_reflection(
    *,
    ...
    # Live port truth only for proxy hint (historical Session-Health-Log must not drive ops).
...
def _probe_http_up(port: int, path: str = "/health", timeout: float = 3.0) -> bool:
    """True if local port answers (health or models)."""
    for p in (path, "/v1/models"):
        try:
...
def _run_active_resident_benchmark(*, timeout: int = 900) -> Dict[str, Any]:
    """Bench the LIVE unified resident on :8090 (fallback :8091).

    RTX 3060 12GB doctrine: never spin a second GGUF / never target legacy
    tier ports 8081/8082. Writes L06-matchable JSON under benchmark-results.
    """
    core = _load_json(CORE_PATH)
    active_path = str(core.get("model") or "")
    active_file = Path(active_path).name if active_path else ""
    if not active_file:
        active_file = str(core.get("model_label") or "active-resident")

    # Prefer llama-server direct; proxy is OK if 8090 briefly flapping
    if _probe_http_up(8090):
        port = 8090
    elif _probe_http_up(8091):
        port = 8091
    else:
        return {
            "ok": False,
    ...
    # Stable stem preferred for L06 matcher + human ops
    stem = "qwythos-9b-abliterated-q6" if "qwythos" in active_file.lower() else Path(active_file).stem
    # Strip weird chars for filename
    stem = "".join(c if c.isalnum() or c in "-_" else "-" for c in stem)[:80]
    out_path = BENCHMARK_DIR / f"{stem}.json"

    script = TOOLBOX["benchmark_resident"]
    if not script.is_file():
        return {"ok": False, "error": "script_missing", "path": str(script)}
    ...
    try:
        ...
        return {"ok": False, "error": "timeout", "message": str(exc), "port": port}
    except Exception as exc:
        return {"ok": False, "error": str(exc), "port": port}

    raw = ((proc.stdout or "") + "\n" + (proc.stderr or "")).strip()
    data = _load_json(out_path) if out_path.is_file() else {}
    # Ensure L06 model key is the GGUF filename even if harness wrote API id
    if data and active_file:
        if str(data.get("model") or "") != active_file:
            data["model"] = active_file
            data["endpoint"] = f"http://127.0.0.1:{port}/v1"
            try:
    ...
    result = {
        ...
        "returncode": proc.returncode,
        "log_tail": raw[-1500:] if raw else "",
    }
    _log({"event": "benchmark_active_resident", **{k: v for k, v in result.items() if k != "log_tail"}})
    return result
...
def _local_smoke_probe(
    ...
    Qwen-family / Mythos-template models may emit tokens only in ``reasoning_content``
    when max_tokens is tiny or thinking is on (empty ``content``). Treat either field
    ...
    # for templates that honor OpenAI-style chat_template_kwargs (harmless if ignored).
    payload = {
        "model": "DEFAULT",
        "messages": [
            {
                "role": "user",
                "content": "Say exactly the two letters OK with no other text.",
            }
        ],
        "max_tokens": 32,
        "temperature": 0,
    ...
    for attempt in range(max(1, retries)):
        ...
        try:
            with urllib.request.urlopen(req, timeout=timeout) as resp:
                ...
                # Inference alive if either channel has tokens (content preferred).
                ok = bool(text or reasoning)
                preview = (text or f"[reasoning] {reasoning}")[:80]
                last = {
                    "ok": ok,
                    "latency_ms": elapsed,
                    "preview": preview,
                ...
                if ok:
                    return last
                last["error"] = "empty_content_and_reasoning"
        except Exception as exc:
            last = {
                "ok": False,
                "error": str(exc),
...
def assess_local_models(core: Dict[str, Any], stack: Dict[str, Any]) -> Dict[str, Any]:
    ...
    gguf_count = len(gguf_truth) if isinstance(gguf_truth, dict) else int(gguf_truth or 0)
    ...
    if BENCHMARK_DIR.is_dir():
        # Prefer newest matching file (mtime) so L06 uses fresh resident runs
        candidates: List[Tuple[float, Path, Dict[str, Any]]] = []
        for path in BENCHMARK_DIR.glob("*.json"):
            data = _load_json(path)
            model_key = str(data.get("model") or path.stem).lower()
        ...
        if candidates:
            ...
            bench_meta = _parse_benchmark_file(best_data, best_path.name)
    ...
    if node.get("up") and not smoke.get("ok"):
        ...
        try:
            ...
            lat = 0
        smoke["slo_warn_ms"] = SMOKE_LATENCY_WARN_MS
        smoke["slo_info_ms"] = SMOKE_LATENCY_INFO_MS
        if lat >= SMOKE_LATENCY_WARN_MS:
            smoke["slo"] = "warn"
            issues.append(
                {
                    "code": "L07S",
        ...
        elif lat >= SMOKE_LATENCY_INFO_MS:
            smoke["slo"] = "info"
            issues.append(
                {
                    "code": "L07S",
...
def run_tick(
    *,
    ...
    all_issues = _annotate_issues(all_issues)
...
def main() -> int:
    ...
    parser.add_argument(
        "--benchmark-active",
        action="store_true",
        help="Resident bench: benchmark_model.py on live :8090/:8091 (never tier :8081)",
    )
    parser.add_argument(
        "--summary",
        action="store_true",
