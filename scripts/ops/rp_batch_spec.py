...
class RenderFrame:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"characters": self.characters, "mode": self.mode, "scene": self.scene, "label": self.label}
...
def build_user_cast_frames(total: int, prompt: str = "", roster: Optional[List[str]] = None, group_size: int = 0) -> List[RenderFrame]:
...
def resolve_series_plan(prompt: str, spec: Optional[dict] = None, total: Optional[int] = None) -> SeriesPlan:
    spec = spec or {}
    gs = infer_group_size(prompt, spec)
    requested = total or gs or 1
    explicit_group = _is_explicit_group(prompt)
    frames = build_user_cast_frames(requested, prompt=prompt, group_size=gs if explicit_group else 1)
    return SeriesPlan(total=len(frames), frames=frames, series="Series" if explicit_group else "Individual")
...
print("Core logic loaded (ASCII only)")
