...
def _batch_caption(png_name: str) -> str | None:
    ...
    series = raw_series
    if "cast" in raw_series.lower() or (raw_series == "Series" and any("Amira & Aisha" in l for l in labels)):
        series = "Series"
    ...
    if index is None or index < 1 or index > total:
        ...
        index = min(delivered + 1, total)
    if index < 1 or index > total:
        index = 1

    raw_label = labels[index - 1]
    label = raw_label
...
def _sync_batch_delivered_count(png_name: str) -> None:
    ...
    if not session.get("active"):
        return
    index = _png_series_index(png_name, session)
    labels_list = list(session.get("labels") or [])
    total = len(labels_list) or int(session.get("total") or 0)
    ...
    labels_list = list(session.get("labels") or [])
    total = len(labels_list) or int(session.get("total") or 0)
    if total > 0 and session["delivered_count"] >= total:
