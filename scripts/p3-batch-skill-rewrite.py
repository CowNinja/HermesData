#!/usr/bin/env python3
"""P3 batch: footers + historical banners on 11 flagged skills."""
...
BANNER = """
> **Historical (2026-06-21):** Any `sovereign_host_*` or `enhanced_voyager_window.py` references below are **archived** side-quest material (`D:\\HermesData\\archive\\deprecated-sovereign-host-training/`). Phase A = Hermes Workspace :3001 + gateway :8642 + PhronesisVault router/MoE. Conductor **parked**.

"""

SKILLS = [
    Path(r"D:\HermesData\skills\agent-swarm-orchestration\SKILL.md"),
    Path(r"D:\HermesData\skills\autonomous-execution-protocol\SKILL.md"),
    Path(r"D:\HermesData\skills\autonomy\autonomous-execution-discipline\SKILL.md"),
    Path(r"D:\HermesData\skills\autonomy\autonomous-self-correction\SKILL.md"),
    Path(r"D:\HermesData\skills\creative\claude-design\SKILL.md"),
    Path(r"D:\HermesData\skills\phronesis-vault-consult\SKILL.md"),
    Path(r"D:\HermesData\skills\productivity\autonomous-execution\SKILL.md"),
    Path(r"D:\HermesData\skills\productivity\living-vault-maintenance\SKILL.md"),
    Path(r"D:\HermesData\skills\productivity\simple-sovereign-work\SKILL.md"),
    Path(r"D:\HermesData\skills\software-development\systematic-debugging\SKILL.md"),
    Path(r"D:\HermesData\skills\vault-documentation-discipline\SKILL.md"),
]
...
REPLACEMENTS = [
    ...
    (
        ...
        "adapt in hermes-workspace or PhronesisVault scripts (not archived sovereign_host_prototype)",
    ),
    (
    ...
    (
        ...
        "For any Hermes recovery, cleanup, or hardening task (Phase A: hermes-workspace + vault CNS, not archived sovereign host):",
    ),
    (
...
def main() -> int:
    for path in SKILLS:
        ...
        if "Phronesis alignment (2026-06-21, P3 batch)" not in text:
            text = text.rstrip() + FOOTER
        if "Historical (2026-06-21):" not in text:
            idx = text.find("\n# ")
            if idx == -1:
                idx = text.find("# ")
            if idx != -1:
                ...
                if end != -1:
                    text = text[: end + 1] + BANNER + text[end + 1 :]
